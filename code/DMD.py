...
class DMD:
    def __init__(self, simu):
        self.X = np.swapaxes(np.reshape(simu.u - simu.umean, (simu.m, simu.h * simu.l)), 0,1)
        self.Y = np.swapaxes(np.reshape(simu.w - simu.wmean, (simu.m, simu.h * simu.l)), 0,1)


    def compute_dmd(self, component = 'u'):
        if component == 'u':
